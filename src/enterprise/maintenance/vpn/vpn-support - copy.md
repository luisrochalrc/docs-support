...
# Connect to your OutSystems Cloud using VPN

This article details how you can set up a site-to-site VPN (virtual private network) connection that allows secure communication between your on-premises network and OutSystems Cloud.

<div class="info" markdown="1">

Check in [Cloud services catalog](https://success.outsystems.com/Support/Enterprise_Customers/OutSystems_Support/Cloud_services_catalog) if the VPN service is available for your OutSystems Cloud edition.
For editions that support VPN connectivity, a maximum of 5 connections can be added.

</div>

## VPN and your OutSystems Cloud

Your OutSystems Cloud infrastructure is hosted on Amazon Web Services (AWS) and protected inside a virtual private cloud (VPC), which is logically isolated from the Internet and from other virtual networks in the AWS cloud.
...
The diagram below shows a single VPN to connect your OutSystems Cloud VPC and your on-premises network.

![VPN connection to OutSystems Cloud](images/vpn-01-vpn-and-your-outsystems-cloud.png)

Each VPN connection consists of two VPN tunnels. The VPN tunnels are anchored to a virtual private gateway on the OutSystems Cloud side, and to your VPN gateway on the on-premises network side.
...
: For devices that does not support assymetric routing is recommended to adopt an Active/Passive approach, with only one tunnel is up at the time.
...
## Before you begin { #setup-procedure }
...
Configure your VPN gateway using the file you downloaded in Step 3.
...
The values provided in the configuration file contains the bare minimum parameters to bring up the VPN. You are able to choose and define another parameters as long as they comply with parameters below.
...
In an architecture of multiple Site-to-Site VPN connections to multiple on-premises locations using Static Routing VPNs, it's not possible to have the same logical routes configured in more than one VPN at the time due to the fact that OutSystems can receive traffic from one tunnel and send it back randomly from another, leading to assymetric routing.

If your use case require such architecture and your gateway does not support assymetric routing is recommended to adopt an Active/Passive approach, with only one tunnel is up at the time.
